...
url = 'https://www.mongolbank.mn/mn/international-reserves'
driver.get(url)

# Find the table with the ID "policy-interest-rate"
table = driver.find_element(By.ID, 'gross-reserves-table')
...
for row in rows:
    ...
    cursor.execute('SELECT COUNT(*) FROM Scrape.[InternationalReservesData] WHERE StatDate = ?', data[0])
    count = cursor.fetchone()[0]

    # Insert the data into the "InflationData" table if it does not exist
    if count == 0:
        cursor.execute('''
            INSERT INTO  Scrape.[InternationalReservesData] (StatDate, TotalNationalResources, MonthlyChange, FromEarlyYear, YearlyChange)
            VALUES (?, ?, ?, ?, ?)
        ''', data )
