...
table = driver.find_element(By.ID, 'policy-interest-rate-table')
...
for row in rows:
    ...
    filtered_data = [(data) for data in data if data is not '']
    
    # Check if the data already exists in the "InflationData" table based on the date
    cursor.execute('SELECT COUNT(*) FROM Scrape.PolicyInterestRateData WHERE StatDate = ?', data[0])
    count = cursor.fetchone()[0]

    # Insert the data into the "InflationData" table if it does not exist
    if count == 0:
        cursor.execute('''
            INSERT INTO  Scrape.PolicyInterestRateData (StatDate, PolicyInterest, Repo, OvernightSaving, RepoFinancing)
            VALUES (?, ?, ?, ?, ?)
        ''', filtered_data)
