...
datetimedd = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    ...
    for package in activated_packages:
        ...
        if os.path.exists(package_path):
            ...
            cursor.execute('''
                           ...
                           ''', package_data[0], datetimedd)
            cursor.execute("SELECT BATCH_LOG_ID FROM BatchLog WHERE BATCH_NAME='Python' AND LOAD_START_DATETIME = ? AND PACKAGE_ACTIVE = 1",datetimedd)
